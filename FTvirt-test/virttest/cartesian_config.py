...
class StrReader(object):
    ...
    def __init__(self, s):
        ...
        for linenum, line in enumerate(s.splitlines()):
            ...
            #if s == 'only FTboot':
            #    sys.stdout.restore()
            #    print "StrReader stripped_line : ",stripped_line,"\n\n"
            #end test
        ...
        #if s == 'only FTboot':
        #    sys.stdout.restore()
        #    print "StrReader self._lines : ",self._lines,"\n\n"
        #    if self._lines[0] == ('only FTboot', 0, 1):
        #        print "\nmatch\n"
        #end test
...
class Parser(object):
    ...
    def parse_string(self, s):
        ...
        #if s == 'only FTboot':
        #    print "s  : ",s,"\n\n"
        #    print "StrReader(s)  : ",dir(StrReader(s)),"\n\n"
        #end test
    ...
    def only_filter(self, variant):
        ...
        #ting test
        #sys.stdout.restore()
        #print "self.get_dicts().next() onlyfiltetr after append :   ",self.get_dicts().next(),"\n\n"
        #end test
        self.parse_string(string)
        #ting test
        #sys.stdout.restore()
        #print "self.get_dicts().next() onlyfiltetr after parser_string :   ",self.get_dicts().next(),"\n\n"
        #end test
    ...
    def get_dicts(self, node=None, ctx=[], content=[], shortname=[], dep=[]):
        ...
        #sys.stdout .restore()
        #print "node :     ",node,"\n\n\n"
        #if len(self.only_filters) == 3  and  self.only_filters[2] ==  'only boot':
        #    print "get_dicts self.only_filters[2] :",self.only_filters[2],"\n\n"
        #end test
        ...
        # ting test
        #a_ = process_content(node.content, new_internal_filters)
        #if a_ == False :
        #    new_internal_filters = []
        #    new_content = []
        #    new_external_filters =[]
        #    c_ = process_content(node.content, new_internal_filters)
        #b_ = process_content(content, new_external_filters)
        #end test
